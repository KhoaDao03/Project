...
_RESEARCH = re.compile(r"\b(search|research|look up|cite|sources?|according to the web|verify online)\b", re.IGNORECASE)
_LOOKUP = re.compile(
...
def needs_current_information(text: str) -> bool:
    ...
    return bool(
        _CURRENT.search(text) or _RESEARCH.search(text)
        or _LOOKUP.search(text) or _MARKET_STATUS.search(text)
    )
