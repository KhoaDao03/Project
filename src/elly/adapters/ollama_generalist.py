...
class OllamaGeneralist:
    ...
    def health(self) -> HealthReport:
        try:
            response = urlopen(
                Request(self._base_url + "/api/tags", method="GET"), timeout=min(5.0, self._timeout)
            )
            with response:
                json.load(response)
    ...
    def generate(self, request: GeneralistRequest) -> GeneralistResponse:
        ...
        return GeneralistResponse(
            ...
            usage=GeneralistUsage(
                output_tokens=len(text.split()), latency_ms=int((time.monotonic() - started) * 1000)
            ),
        )
