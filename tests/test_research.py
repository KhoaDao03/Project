...
class CitationValidatorTests(unittest.TestCase):
    ...
    def test_resolved_private_address_is_rejected(self) -> None:
        ...
        result = validate_citations((ProviderCitation("https://example.com/private"),), resolver=resolver, resolve_hosts=True)
        self.assertEqual(result.evidence, ())


class EvidenceSelectionTests(unittest.TestCase):
    @staticmethod
    def item(identifier: str, text: str, *, url: str | None = None, days_old: int = 0,
             source_class: str = "secondary") -> EvidenceObject:
        return EvidenceObject(
            evidence_id=identifier, url=url or f"https://example.com/{identifier}",
            canonical_url=url or f"https://example.com/{identifier}", title=text,
            publisher="Fixture", snippet=text, retrieved_at=UTC - timedelta(days=days_old),
    ...
    def test_token_budget_evicts_lower_rank_without_overrun(self) -> None:
        result = select_evidence(
            ...
            ), now=UTC, current_information=False, token_budget=6,
        )
...
class ResearchPipelineTests(unittest.TestCase):
    ...
    def test_conflicting_provider_answer_is_not_known(self) -> None:
        provider = FixtureWebResearchProvider(
            ...
            citations=(ProviderCitation("https://example.com/one", "One", snippet="Value A."), ProviderCitation("https://example.com/two", "Two", snippet="Value B.")),
        )
    ...
    def test_bare_urls_do_not_fabricate_claim_support(self) -> None:
        provider = FixtureWebResearchProvider(
            ...
            citations=(ProviderCitation(
                "https://example.com/source", "latest result metadata"
            ),),
        )
        outcome = ResearchPipeline(provider=provider, clock=_Clock(), max_results=5, timeout_seconds=1).execute("latest result")
    ...
    def test_unverified_summary_quarantines_instructions_and_free_urls(self) -> None:
        provider = FixtureWebResearchProvider(
            ...
            citations=(ProviderCitation(
                "https://example.com/source", "useful preliminary answer"
            ),),
        )
        outcome = ResearchPipeline(
            provider=provider, clock=_Clock(), max_results=5, timeout_seconds=1
    ...
    def test_conflicting_unverified_summary_remains_unknown(self) -> None:
        provider = FixtureWebResearchProvider(
            ...
            citations=(ProviderCitation(
                "https://example.com/source", "latest result"
            ),),
        )
        outcome = ResearchPipeline(
            provider=provider, clock=_Clock(), max_results=5, timeout_seconds=1
...
class OpenAIHostedWebSearchTests(unittest.TestCase):
    ...
    def http_error(code: int, error_code: str = "") -> urllib.error.HTTPError:
        ...
        return urllib.error.HTTPError("https://api.openai.com/v1/responses", code, "error", {}, BytesIO(body))
...
class ResearchCliTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.old = {name: os.environ.get(name) for name in (
            "ELLY_DB_PATH", "ELLY_LOG_LEVEL", "ELLY_GENERALIST_PROVIDER",
            "ELLY_GENERALIST_MODEL_ID", "ELLY_RESEARCH_PROVIDER",
        )}
        os.environ.update({
    ...
    def test_cli_displays_unverified_summary_with_validated_sources(self) -> None:
        ...
        self.app.research.provider.citations = (
            ProviderCitation(
                "https://www.example.com/current", "current result metadata"
            ),
        )
        self.cli.dispatch("/mode cloud")
